...
#     val_split = 0.1
    ...
    ###### mlflow ###### 
    mlflow.keras.autolog() # mlflow 자동로그
    # Hyperparameter search space
    search_space = {
    #     "optimizer": hp.choice('optmz',["sgd", "rms"]),
        "epochs": hp.choice('epochs', [20, 30, 40, 50, 60]),
...
#             history = model.fit_generator(data_generator_wrapper(train_data, batch_size, input_shape, anchors, num_classes),
...
#             ###### mlflow ######
#             for i in range(len(model_body.layers)-12,len(model.layers)): #shjeong changed
#                 model.layers[i].trainable = True
#             model.compile(optimizer=Adam(lr=lr), loss={'yolo_loss': lambda y_true, y_pred: y_pred}) # recompile to apply the change
#             print('Unfreeze last 12 layers') #conv 구역 2번 포함하는 범위

#             batch_size = 32 # note that more GPU memory is required after unfreezing the body
#             print('Train on {} samples, val on {} samples, with batch size {}.'.format(num_train, num_val, batch_size))
#             history2 = model.fit_generator(data_generator_wrapper(train_data, batch_size, input_shape, anchors, num_classes),
#                 steps_per_epoch=max(1, num_train//batch_size),
#                 validation_data=data_generator_wrapper(val_data, batch_size, input_shape, anchors, num_classes),
